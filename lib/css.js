...
function parseNode(node, base) {
  ...
  if (
    ...
    node.type === 'host'
      ...
      return node[node.type].rules.reduce((all, rule) => {
        return all.concat(parseNode(rule, base))
      }, [])
  }
  ...
  if (
    ...
    return node.declarations.reduce((all, declaration) => {
      return all.concat(parseNode(declaration, base))
    }, [])
  }

  if (node.type === 'keyframes') {
    return node.keyframes.reduce((all, keyframe) => {
      return all.concat(parseNode(keyframe, base))
    }, [])
  }
...
module.exports = function reCSS(file) {
  return readFile(file)
    .then((source) => {
      ...
      const dependencies = parseNode(ast, file)
